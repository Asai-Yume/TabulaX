...
for %%S in (0 1 2 3 4) do (
  for /D %%D in (data\ss\*) do (
    python -c "import pathlib,sys; p=pathlib.Path(r'outputs\ss_%%~nxD_fixed10_bothmatch_gpt5mini_sweep_seed%%S\_res.csv'); lines=p.read_text(encoding='utf-8', errors='replace').splitlines() if p.exists() else []; complete=(len(lines)>=3) or (len(lines)>=2 and any(',num_dist,Numbers,' in line for line in lines[1:])); sys.exit(0 if complete else 1)"
    ...
    if errorlevel 1 (
      ...
      set "TABULAX_DS_NAME=ss_%%~nxD_fixed10_bothmatch_gpt5mini_sweep_seed%%S"
      set "TABULAX_OUTPUT_DIR=%CD%\outputs\ss_%%~nxD_fixed10_bothmatch_gpt5mini_sweep_seed%%S"
